...
import { PropAnalysisResult, PropPattern, PropUsage, ComponentUsage } from '../../core/PropAnalyzer';
  ...
  components: [
    {
      ...
      props: [
        { name: 'userId', type: 'string', required: true, valueChanges: 5, usageCount: 10 },
        { name: 'userData', type: 'object', required: true, valueChanges: 15, usageCount: 10 }
      ]
    },
    {
      componentName: 'Dashboard',
      props: [
        { name: 'data', type: 'array', required: true, valueChanges: 20, usageCount: 10 },
        { name: 'loading', type: 'boolean', required: false, valueChanges: 8, usageCount: 10 }
      ]
    }
  ],
  unusedProps: [
    { componentName: 'UserProfile', propName: 'theme', updateCount: 0 },
    { componentName: 'Dashboard', propName: 'debug', updateCount: 0 }
  ],
  propPatterns: [
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setData((prev: PropAnalysisResult) => ({
        ...prev,
        components: prev.components.map((component: ComponentUsage) => ({
          ...component,
