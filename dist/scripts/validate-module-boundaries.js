...
const validateModuleBoundaries = async (directory) => {
    const violations = [];
    ...
    for (const file of files) {
        ...
        if (!relativePath.startsWith(MODULES.TOOLS) && containsRestrictedPatterns(content)) {
            ...
            if (file.endsWith('validate-module-boundaries.ts')) {
                continue;
            }
            violations.push({
...
// Run validation
async function validateModuleBoundariesScript(directory = 'src') {
    try {
...
//# sourceMappingURL=validate-module-boundaries.js.map