 ...
 */
export function createMovingAverage(windowSize) {
    const window = [];
    return (value) => {
        window.push(value);
...
export function detectAnomalies(values, threshold = 2) {
    ...
    const stdDev = Math.sqrt(values.reduce((sum, val) => sum + Math.pow(val - summary.avg, 2), 0) / values.length);
 ...
 */
export function visualizeDistribution(values, buckets = 10) {
    if (values.length === 0) {
 ...
 */
export function calculateThroughput(timestamps, windowSize = 1000) {
 ...
 */
export function calculateStatisticalMoments(values) {
 ...
 */
export function analyzeTrend(values, timestamps) {
    const n = values.length;
 ...
 */
export function decomposeTimeSeries(values, period) {
    const n = values.length;
    if (n < period * 2) {
...
export function analyzeLatencyPatterns(latencies, timestamps) {
    const patterns = [];
    const bottlenecks = [];
    const statistics = {};
    ...
    if (timestamps && timestamps.length === latencies.length) {
        ...
        if (statistics.trend.confidence > 0.95) {
            patterns.push(`Strong ${statistics.trend.trend} trend detected (confidence: ${(statistics.trend.confidence * 100).toFixed(1)}%)`);
        }
        ...
        if (latencies.length >= 24) {
            try {
                ...
                const seasonalStrength = Math.abs(statistics.seasonal.seasonal.reduce((a, b) => a + Math.abs(b), 0) /
                    latencies.length);
 ...
 */
export function visualizeTimeSeries(values, timestamps, height = 20, width = 80) {
    if (values.length === 0)
        return 'No data to visualize';
    ...
    // Add axis labels
    const yAxisLabels = Array(5).fill(0).map((_, i) => (valueMin + (valueRange * i / 4)).toFixed(1).padStart(8));
    ...
    const xLabels = Array(5).fill(0).map((_, i) => Math.floor(timeMax * i / 4).toString().padStart(Math.floor(width / 4))).join('');
    return [
        ...plotLines,
        xAxis,
        '         ' + xLabels
 ...
 */
export function visualizeCorrelation(values1, values2, height = 20, width = 80) {
    if (values1.length === 0 || values2.length === 0)
        return 'No data to visualize';
    if (values1.length !== values2.length)
        return 'Data series must have equal length';
    ...
    const yLabels = Array(5).fill(0).map((_, i) => (y1 + ((y2 - y1) * i / 4)).toFixed(1).padStart(8));
    const plotLines = plot.map((line, i) => {
        const yLabel = i % Math.floor(height / 4) === 0
    ...
    const xAxis = '        └' + '─'.repeat(width);
    const xLabels = Array(5).fill(0).map((_, i) => (x1 + ((x2 - x1) * i / 4)).toFixed(1).padStart(Math.floor(width / 4))).join('');
    return [
...
export function generatePerformanceReport(metrics) {
    ...
    if (metrics.timestamps && metrics.timestamps.length > 0) {
        sections.push('', 'Latency Over Time:', visualizeTimeSeries(metrics.latencies, metrics.timestamps), '', 'Throughput vs Latency Correlation:', visualizeCorrelation(metrics.throughput, metrics.latencies));
        if (analysis.statistics?.seasonal) {
            sections.push('', 'Seasonal Decomposition:', 'Trend Component:', visualizeTimeSeries(analysis.statistics.seasonal.trend, metrics.timestamps), '', 'Seasonal Component:', visualizeTimeSeries(analysis.statistics.seasonal.seasonal, metrics.timestamps));
        }
    }
    sections.push('', 'Analysis:', ...analysis.patterns);
